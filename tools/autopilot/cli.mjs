#!/usr/bin/env node
/**
 * CLI Autopilot CINs — Giai đoạn 1 (khung).
 *
 * Usage:
 ...
 *   npm run autopilot -- tao-viec --loai quet_nguon [--payload '{"idNguon":"..."}']
 */
...
function inHuongDan() {
  console.log(`Autopilot CLI (Giai đoạn 1)

Lệnh:
  trang-thai                         Đếm bảng + nick + env
  dong-bo-nick                       Upsert 10 nick seeding → auto_tai_khoan
  liet-ke <nguon|nick|muc>           Liệt kê
  them-nguon --nen-tang <t> --url <u> [--ma-ngoai] [--ten] [--niche] [--ghi-chu]
  tao-viec --loai <loai> [--payload JSON] [--hen-luc ISO]
