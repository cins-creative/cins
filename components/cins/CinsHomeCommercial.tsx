...
/** Nhóm ngành thị giác — ticker dưới hero scrim (theo bảng ten_en + nhóm bổ sung) */
const HERO_VISUAL_INDUSTRY_TICKER = [
  "Game",
  "Post-production",
  "Industrial Design",
  "Motion Design",
...
/** Ảnh nền hero — xoay ~5s / ảnh (quảng cáo cover) */
const HERO_COVER_ADS = [
  {
    src: "https://maac.edu.vn/wp-content/uploads/2025/04/MAAC_Post_Chao2025_250505_1400x527.jpg",
    alt: "",
  },
  {
...
const HERO_COVER_ROTATE_MS = 5000;
...
function Hero() {
  ...
  useEffect(() => {
    if (reduceMotion || HERO_COVER_ADS.length <= 1) return;
    const id = window.setInterval(() => {
      setCoverIndex((i) => (i + 1) % HERO_COVER_ADS.length);
    }, HERO_COVER_ROTATE_MS);
  ...
  return (
    <section className="hero-cmm">
      {/* Cover + copy */}
      <div className="hero-cmm-visual">
        <div className="hero-cmm-bg">
          <div className="hero-cmm-bg-slides" aria-hidden="true">
            {HERO_COVER_ADS.map((ad, idx) => (
              <div
                key={ad.src}
                className={
                  "hero-cmm-bg-slide" +
                  (idx === coverIndex ? " hero-cmm-bg-slide--active" : "")
                }
              >
                <Image
                  src={ad.src}
          ...
          <div className="hero-cmm-scrim" />
          <div
            className={
              "hero-cmm-ticker-wrap" +
              (reduceMotion ? " hero-cmm-ticker-wrap--static" : "")
            }
            aria-label="Các nhóm ngành thị giác"
          >
            {reduceMotion ? (
              <ul className="hero-cmm-ticker-static">
                {HERO_VISUAL_INDUSTRY_TICKER.map((label) => (
                  <li key={label} className="hero-cmm-ticker-static-li">
                    {label}
                  </li>
                ))}
              </ul>
            ) : (
              <div className="hero-cmm-ticker">
                <div className="hero-cmm-ticker-track">
                  {[
                    ...HERO_VISUAL_INDUSTRY_TICKER,
                    ...HERO_VISUAL_INDUSTRY_TICKER,
                  ].map((label, i) => (
                    <span
                      className="hero-cmm-ticker-item"
                      key={`${label}-${i}`}
                    >
                      <span className="hero-cmm-ticker-text">{label}</span>
                      <span className="hero-cmm-ticker-dot" aria-hidden />
                    </span>
                  ))}
                </div>
              </div>
        ...
        <div className="page hero-cmm-search-stack">
          <div className="hero-cmm-search">
            ...
            <button type="button" className="hero-cmm-search-btn">
              Tìm kiếm
            </button>
          </div>
        </div>
  ...
  const tiles = [
    {
      ...
      tint:
        "linear-gradient(135deg, rgba(31,116,201,0.10), rgba(31,116,201,0.04))",
      title: "Khám phá nghề",
    ...
    },
    {
      ...
      tint:
        "linear-gradient(135deg, rgba(110,254,192,0.20), rgba(110,254,192,0.04))",
    ...
    },
    {
      ...
      tint:
        "linear-gradient(135deg, rgba(255,184,92,0.20), rgba(255,184,92,0.04))",
          ...
          {tiles.map((t, i) => (
            <div className="ft-card" key={i}>
              <div className="ft-card-img" style={{background: t.tint}}>
                <img src={t.img} alt="" />
                <div className="ft-card-ic">
                  <MsIcon name={t.iconGlyph} />
                </div>
              </div>
    ...
    <section className="evb-section">
      <div className="page">
        <div className="evb-head">
          ...
          <a href="/events" className="evb-link">Xem tất cả sự kiện →</a>
        </div>
        <div className="evb-grid">
          {events.map((e, i) => (
            <a key={i} href="#" className={`evb-card ${e.featured ? 'is-featured' : ''}`}>
              <div className="evb-card-img relative">
                <Image src={e.img} alt="" fill className="object-cover" sizes="(max-width: 900px) 100vw, 33vw" />
...
function DevChannelBadge() {
  if (process.env.NODE_ENV !== "development") return null;
  return (
    <div className="fixed bottom-3 right-3 z-[300] rounded-full border border-neutral-200 bg-white px-3 py-1.5 text-xs font-semibold text-neutral-600 shadow-md">
      Kênh: localhost:3001
    </div>
  );
}

export function CinsHomeCommercial() {
  return (
    <>
      <DevChannelBadge />
      <CinsShell data-screen-label="01 Homepage">
        <Hero />
        <FeatureTiles />
        <EventBanners />
        <BigSplit />
