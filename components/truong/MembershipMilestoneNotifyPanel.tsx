...
type MembershipMilestoneNotifyPanelProps = {
  orgId: string;
  active: boolean;
...
export function MembershipMilestoneNotifyPanel({
  orgId,
  active,
  onPendingCountChange,
  onToast,
}: MembershipMilestoneNotifyPanelProps) {
  ...
  const loadItems = useCallback(
    async (options?: { silent?: boolean }) => {
      ...
      try {
        const res = await fetch(`/api/org/${orgId}/membership-milestone-requests`, {
          cache: "no-store",
        });
  ...
  async function respondRequest(id: string, action: "approve" | "reject") {
    try {
      const res = await fetch(
        `/api/org/${orgId}/membership-milestone-requests/${id}`,
        {
          method: "PATCH",
      ...
      if (!res.ok) {
        onToast(json.error ?? "Không cập nhật được.");
        return;
      }
      ...
      onToast(action === "approve" ? "Đã xác thực cột mốc" : "Đã từ chối yêu cầu");
    } catch {
      onToast("Lỗi mạng.");
    }
  }
  ...
  return (
    <>
      ...
      <div
        ...
        role="tablist"
        aria-label="Lọc trạng thái xác thực"
      >
        {(
          [
      ...
      >
        <aside className="tdh-milestone-tag-list-pane" aria-label="Danh sách yêu cầu xác thực">
          <ul className="tdh-milestone-tag-rows">
            {loading ? (
              ...
              <li className="tdh-milestone-tag-empty">Không có mục trong bộ lọc này.</li>
            ) : (
              filtered.map((row) => (
                <MembershipListRow
                  key={row.id}
