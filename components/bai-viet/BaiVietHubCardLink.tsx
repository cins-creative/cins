...
export const BaiVietHubStaticTooltipContext = createContext(false);

export function useBaiVietHubStaticTooltip(): boolean {
  return useContext(BaiVietHubStaticTooltipContext);
}
...
type Props = {
  ...
  className?: string;
  children: ReactNode;
} & Omit<ComponentPropsWithoutRef<typeof Link>, "href" | "children" | "className" | "aria-label">;
...
/** Link thẻ hub bài viết — mô tả chỉ hiện tooltip khi hover (giống /nghe-nghiep). */
export function BaiVietHubCardLink({
  href,
  ariaLabel,
  tooltip,
  className,
  ...
  const useCursorTooltip = finePointer && !!tip;
  ...
  }, [hovering, useCursorTooltip, tip, updatePosition]);

  const showStaticTooltip = !!tip && !useCursorTooltip;

  return (
    <>
      <BaiVietHubStaticTooltipContext.Provider value={showStaticTooltip}>
        <Link
          href={href}
          className={className}
      ...
      </BaiVietHubStaticTooltipContext.Provider>
