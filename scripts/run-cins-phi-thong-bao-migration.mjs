/**
 * Chạy migration_cins_phi_thong_bao.sql
 * Usage: npm run migrate:cins-phi-thong-bao
 */
...
const sqlPath = path.join(
  __dirname,
  "../supabase/sql/migration_cins_phi_thong_bao.sql",
);
...
console.log("CREATE cins_phi_thong_bao …");
...
try {
  ...
  const [v] = await db`
    SELECT
      (SELECT to_regclass('public.cins_phi_thong_bao') IS NOT NULL) AS co_bang
  `;
  console.log("OK: cins_phi_thong_bao migration");
  console.log(`Verify — bang=${v.co_bang}`);
} catch (err) {
