/**
 * Chạy migration_shop_buyer_soft_limit_p3b.sql
 * Usage: npm run migrate:shop-buyer-soft-limit
 */
...
const sqlPath = path.join(
  __dirname,
  "../supabase/sql/migration_shop_buyer_soft_limit_p3b.sql",
);
  ...
  const [v] = await db`
    SELECT EXISTS (
      ...
      WHERE table_schema='public' AND table_name='cins_cau_hinh_tai_chinh'
        AND column_name='buyer_toi_da_don_moi_ngay'
    ) AS ok
  `;
  console.log("OK: shop_buyer_soft_limit_p3b", `col=${v.ok}`);
} catch (err) {
