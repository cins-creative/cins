/**
 * Chạy migration_chat_canvas_che_do_sao_chep.sql
 * Usage: npm run migrate:chat-canvas-che-do-sao-chep
 */
...
const sqlPath = path.join(
  __dirname,
  "../supabase/sql/migration_chat_canvas_che_do_sao_chep.sql",
);
    ...
    SELECT
      EXISTS (
        ...
        WHERE table_schema = 'public'
          AND table_name = 'chat_canvas'
          AND column_name = 'che_do_sao_chep'
      ) AS col_ok,
      EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'chat_canvas_che_do_sao_chep_check'
      ) AS check_ok
  `;
  console.log(
    "OK: chat_canvas_che_do_sao_chep",
    `col=${v.col_ok}`,
    `check=${v.check_ok}`,
  );
} catch (err) {
