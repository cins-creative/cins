...
const dir = join(import.meta.dirname, "mon-hoc-content");
...
for (const job of items) {
  ...
  const tieuDeEng = (job.tieu_de_eng ?? "").replace(/'/g, "''");

  const sql = `
UPDATE article_bai_viet SET
  tieu_de = '${job.tieu_de.replace(/'/g, "''")}',
  tieu_de_eng = ${job.tieu_de_eng ? `'${tieuDeEng}'` : "NULL"},
...
const remain = await runAdminSql(
  `SELECT COUNT(*) AS con_lai FROM article_bai_viet
   WHERE loai_bai_viet = 'mon_hoc' AND (noi_dung IS NULL OR noi_dung = '')`,
  "read",
);
