/**
 * Gộp article_bai_viet.loai_bai_viet: phan_mem → keyword.
 * Usage: node scripts/run-phan-mem-to-keyword-migration.mjs
 *        npm run migrate:phan-mem-to-keyword
 */
...
const sqlPath = path.join(
  __dirname,
  "../supabase/sql/migration_phan_mem_to_keyword.sql",
);
...
try {
  ...
  const counts = await db`
    SELECT
      count(*) FILTER (WHERE loai_bai_viet = 'phan_mem')::int AS phan_mem,
      count(*) FILTER (WHERE loai_bai_viet = 'keyword')::int AS keyword
    FROM public.article_bai_viet
  `;
  console.log("OK: phan_mem → keyword applied");
