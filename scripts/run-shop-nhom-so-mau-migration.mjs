/**
 * Chạy migration_shop_nhom_so_mau.sql trên CINS.
 * Usage: npm run migrate:shop-nhom-so-mau
 */
...
const sqlPath = path.join(
  __dirname,
  "../supabase/sql/migration_shop_nhom_so_mau.sql",
);
...
try {
  ...
  const [{ count }] = await db`
    select count(*)::int as count from shop_nhom where so_mau > 0
  `;
  console.log(`OK: shop_nhom.so_mau + trg_shop_san_pham_so_mau`);
  console.log(`Backfill: ${count} loại có so_mau > 0`);
} catch (err) {
