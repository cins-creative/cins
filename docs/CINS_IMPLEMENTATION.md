...
| File | Tạo gì |
|---|---|
| `migration_article_gan_linh_vuc.sql` | **Nghề ↔ nhiều lĩnh vực:** bảng `article_gan_linh_vuc` (M:N, `la_chinh`) + backfill từ `article_bai_viet.id_linh_vuc` + RLS SELECT public. Hub filter + hero chip đọc junction. Merge mẫu: `npx tsx scripts/merge-nghe-3d-animator.mts` → slug `nghe-3d-animator`. Chạy: `node scripts/run-article-gan-linh-vuc-migration.mjs`. Plan: `docs/PLAN_nghe_merge_da_linh_vuc.md`. |
