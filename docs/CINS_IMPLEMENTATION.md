...
| `to-chuc/` | **Cơ sở đào tạo** — trang chi tiết, settings, members, filters, timeline lớp, staff invite, create; **sự kiện + loại vé** | `co-so-page-queries.ts`, `co-so-settings.ts`, `co-so-members.ts`, `co-so-create.ts`, `co-so-vai-tro.ts`, `su-kien.ts`, `su-kien-loai-ve.ts`, `su-kien-constants.ts`, `su-kien-listing.ts` |
