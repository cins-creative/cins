...
| `migration_user_gallery_noi_bat.sql` | **Thứ tự Nội dung nổi bật (cột aside Journey):** bảng `user_gallery_noi_bat` (`id_nguoi_dung`, `id_cot_moc`, `thu_tu`, PK cặp). Chỉ chủ Journey `PATCH /api/journey/[slug]/gallery-aside` `{ cotMocIds }`; mọi viewer đọc cùng thứ tự. Không đổi sort Gallery chính. App: `lib/journey/gallery-noi-bat-order.ts`, DnD trong `JourneyGalleryAside` (`featuredOnly` + `canReorder`). Chạy: `node scripts/run-user-gallery-noi-bat-migration.mjs`. |
