...
- **World Timeline scoring (L30):** bảng `content_diem_feed` (`migration_content_diem_feed.sql`, runner `scripts/run-content-diem-feed-migration.mjs`). Lib: `lib/cins/feed-scoring.ts` (công thức / `FEED_SCORE` / `breakdownDiemHienTai` / `gioConLaiDecay`) · `feed-scoring-load.ts` (load điểm 7 ngày + gắn feed) · `feed-scoring-write.ts` (upsert publish/boost/verify/nội dung + `markEngagementDirty` + **lazy** `flushDirtyEngagementScores`, **cap 40 dirty/request**). Backfill one-time: `scripts/backfill-content-diem-feed.mjs` (`npm run backfill:diem-feed`). Đối tượng: `cot_moc` + `org_bai_dang` — không `org_su_kien`. Pipeline: score → author echo → soft quota → boost merge. Chi tiết hằng số / scale 3 tầng → DECISIONS L30.
- **World editorial boost (L29):** bảng `content_world_boost` (`migration_content_world_boost.sql`, runner `scripts/run-content-world-boost-migration.mjs`). Lib: `lib/cins/world-boost.ts` · `world-boost-admin.ts` · `world-boost-client.ts`. API: `GET|POST /api/admin/world-boost` (gate `canManageUsers`). Merge sau cache trong `worldJourneyFeedFetch` / `worldJourneyGalleryFetch`. UI: `/admin/noi-dung-dang` (Grid/Listing hiện điểm L30) + toggle trên World (`WorldBoostAdminProvider`). Cap rank `WORLD_BOOST_RANK_CAP=15`. TTL 3 ngày tự gia hạn; bật đẩy → upsert điểm Timeline (base 100).
