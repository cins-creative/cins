...
/** Polymorphic target dùng chung `social_binh_luan` cho bài đăng tổ chức. */
export const ORG_BAI_DANG_COMMENT_LOAI = SOCIAL_LOAI_ORG_BAI_DANG;
export const ORG_BAI_DANG_COMMENT_MAX_LEN = 1000;
...
/** Số bình luận gốc (chưa xoá) theo từng bài đăng — hydrate nút bình luận. */
export async function fetchOrgBaiDangCommentCounts(
  postIds: string[],
  ...
  const { data, error } = await admin
    ...
    .select("id_doi_tuong")
    .eq("loai_doi_tuong", ORG_BAI_DANG_COMMENT_LOAI)
...
/** Cây bình luận của một bài đăng tổ chức cho viewer hiện tại. */
export async function fetchOrgBaiDangComments(
  postId: string,
  viewerId: string | null,
): Promise<MilestonePostComment[]> {
  return fetchCommentsForSocialObject(
    ORG_BAI_DANG_COMMENT_LOAI,
    postId,
    viewerId,
  );
}

/** Ghi một bình luận trên bài đăng tổ chức (`org_bai_dang`). */
export async function insertOrgBaiDangComment(input: {
  postId: string;
...
> {
  ...
  if (text.length > ORG_BAI_DANG_COMMENT_MAX_LEN) {
    return {
      ok: false,
      message: `Bình luận tối đa ${ORG_BAI_DANG_COMMENT_MAX_LEN} ký tự.`,
    };
  }
  ...
  if (!post || post.trang_thai !== "da_dang") {
    return { ok: false, message: "Bài đăng không tồn tại." };
  }
  ...
  if (input.idCha) {
    ...
    if (!target || target.da_xoa || target.id_doi_tuong !== input.postId) {
  ...
  const { data, error } = await admin
    ...
    .insert({
      ...
      loai_doi_tuong: ORG_BAI_DANG_COMMENT_LOAI,
      id_doi_tuong: input.postId,
      noi_dung: text,
      id_cha: idCha,
