...
/** `ma_lop` sinh tự động (`buildMaLop`) — chưa phải mã lớp do admin đặt. */
export function isAutoGeneratedMaLop(maLop: string): boolean {
  const trimmed = maLop.trim();
  if (!trimmed) return true;
  return /-\d{8}$/.test(trimmed);
}

export type ScaffoldLopDbRow = {
  ma_lop?: string | null;
  giao_vien_phu_trach?: string | null;
  giao_vien_text?: string | null;
};

/** Lớp scaffold trong DB — tự sinh khi tạo khóa, chưa cấu hình mã / GV. */
export function isScaffoldLopDbRow(row: ScaffoldLopDbRow): boolean {
  const ma = row.ma_lop?.trim() ?? "";
  if (ma && !isAutoGeneratedMaLop(ma)) return false;
  if (row.giao_vien_text?.trim()) return false;
  if (row.giao_vien_phu_trach?.trim()) return false;
  return true;
}
