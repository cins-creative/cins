...
export async function listLopHocQuanLy(
  ...
  const { data: lopRows, error } = await admin
    .from("org_lop_hoc")
    .select(
      "id, ma_lop, hinh_thuc, lich_hoc, ngay_khai_giang, slot_toi_da, trang_thai, giao_vien_phu_trach, giao_vien_text, id_khoa_hoc, id_chat_phong",
    )
  ...
  const chatRoomIds = [
    ...new Set(
      list
        .map((l) => l.id_chat_phong as string | null)
  ...
  const gvById = new Map(
    (gvRes.data ?? []).map((u) => {
      ...
      const avatarId = (u.avatar_id as string | null) ?? null;
      return [
        id,
        {
          ten: (u.ten_hien_thi as string) || "Giảng viên",
  ...
  return list.map((row) => {
    ...
    const gv = gvId ? gvById.get(gvId) : undefined;
    ...
    return {
      ...
      giaoVienText: gvText,
      giaoVienTen: gv?.ten ?? gvText,
      giaoVienSlug: gv?.slug ?? null,
      giaoVienAvatarId: gv?.avatarId ?? null,
      giaoVienAvatarUrl: gv?.avatarUrl ?? null,
