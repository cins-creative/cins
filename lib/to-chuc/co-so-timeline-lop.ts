...
/** Nhãn lớp trên timeline / sidebar — ưu tiên mã admin, không hiển thị slug tự sinh. */
export function resolveCoSoLopTimelineLabel(pin: CoSoLopTimelinePin): string {
  const ma = pin.maLop?.trim();
  if (ma && !isAutoGeneratedMaLop(ma)) return ma;
  return pin.tenKhoaHoc?.trim() || "Lớp học";
}
