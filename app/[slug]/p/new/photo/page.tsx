...
export const metadata: Metadata = {
  title: "Thêm ảnh · CINS",
  description: "Đăng ảnh lên Journey trên CINs.",
  robots: { index: false, follow: false },
};

export default async function NewPhotoPage({ params }: { params: Params }) {
  const { slug } = await params;
  const session = await getCurrentSessionAndProfile();
  if (!session) {
    redirect(`/login?next=${encodeURIComponent(`/${slug}/p/new/photo`)}`);
  }
