...
import { OrgOgShareCard } from "@/lib/og/org-og-card";
import { fetchTruongOgContext } from "@/lib/truong/truong-og-fetch";

export const size = { width: 1200, height: 630 };
export const contentType = "image/png";
export const alt = "Trường đào tạo ngành sáng tạo trên CINs";
...
export default async function Image({ params }: { params: Params }) {
  ...
  const [ctx, fonts] = await Promise.all([
    fetchTruongOgContext(slug),
    loadOgFonts(),
  ]);

  const element = ctx ? (
    <OrgOgShareCard ctx={ctx} slug={slug} logoUrl={logoUrl} />
  ) : (
    <div
      style={{
        width: "100%",
    ...
    >
      ...
      <div style={{ fontSize: 24, fontWeight: 600, color: "#64748b" }}>
        Ngành sáng tạo thị giác trên CINs
      </div>
    </div>
  );
