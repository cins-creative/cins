...
import {
  deleteShopDangKyMoAdmin,
  updateShopDangKyMoAdmin,
} from "@/lib/shop/dang-ky-mo-admin";
...
export async function adminGoShopDangKyMo(
  id: string,
): Promise<{ ok: true } | { ok: false; message: string }> {
  if (!hasServiceRoleEnv()) {
    return { ok: false, message: "Thiếu SUPABASE_SERVICE_ROLE_KEY trên server." };
  ...
  const result = await deleteShopDangKyMoAdmin(id);
  if (!result.ok) return { ok: false, message: result.error };

  revalidatePath("/admin/mo-shop");
  revalidatePath(`/admin/mo-shop/${id.trim()}`);
  revalidatePath("/mo-shop");
  return { ok: true };
}
