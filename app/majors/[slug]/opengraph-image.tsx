...
import { NganhOgShareCard } from "@/lib/nganh/nganh-og-card";
import { fetchNganhOgContext } from "@/lib/nganh/nganh-og-fetch";

export const size = { width: 1200, height: 630 };
export const contentType = "image/png";
export const alt = "Ngành đào tạo ngành sáng tạo thị giác trên CINs";
...
export default async function Image({ params }: { params: Params }) {
  ...
  const [ctx, fonts] = await Promise.all([
    fetchNganhOgContext(slug),
    loadOgFonts(),
  ]);

  const element = ctx ? (
    <NganhOgShareCard ctx={ctx} slug={slug} logoUrl={logoUrl} />
  ) : (
    <div
      style={{
        width: "100%",
    ...
    >
      ...
      <div style={{ fontSize: 24, fontWeight: 600, color: "#64748b" }}>
        Chọn đúng ngành đại học trên CINs
      </div>
    </div>
  );
