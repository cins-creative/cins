...
export async function CoSoDetailLoader({ slug }: Props) {
  ...
  const [canEdit, canManageKhoaHoc, bookmarkSocial, khoaHocResult] =
    await Promise.all([
      ...
      listKhoaHocCuaOrg(payload.school.id),
    ]);

  const initialKhoaHoc = khoaHocResult.ok ? khoaHocResult.khoaHoc : [];
