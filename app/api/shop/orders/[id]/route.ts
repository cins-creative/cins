...
export async function PATCH(request: Request, ctx: Ctx) {
  ...
  try {
    ...
    if (body.action === "yeu_cau_huy") {
      const lyDo =
        typeof body.lyDo === "string" ? body.lyDo.trim().slice(0, SHOP_LY_DO_HUY_MAX) : "";
      if (!lyDo) {
        return NextResponse.json(
          { error: "Cần nêu lý do nhờ khách hủy đơn." },
          { status: 422 },
        );
      }
      const don = await yeuCauHuyDonHang(session.profile.id, id, lyDo);
      return NextResponse.json({ don, chatContext: donHangToChatContext(don) });
    }
    if (body.action === "bo_yeu_cau_huy") {
      const don = await huyYeuCauHuyDonHang(session.profile.id, id);
      return NextResponse.json({ don, chatContext: donHangToChatContext(don) });
    }
    if (body.action === "buyer_huy") {
      const lyDo =
        typeof body.lyDo === "string" ? body.lyDo.trim().slice(0, SHOP_LY_DO_HUY_MAX) : "";
      const don = await cancelDonHangByBuyer(
        session.profile.id,
        id,
        lyDo || null,
      );
    ...
    if (msg === "NEED_SHOP_REQUEST") {
      return NextResponse.json(
        {
          error:
            "Đơn đã được shop xác nhận — nhờ shop gửi yêu cầu hủy trong chat trước.",
        },
        { status: 422 },
      );
    }
    ...
    if (msg === "KHIEU_NAI_MO") {
      return NextResponse.json(
        { error: "Đơn đang khiếu nại — chờ admin xử lý." },
        { status: 422 },
      );
    }
    if (msg === "NOT_YET") {
