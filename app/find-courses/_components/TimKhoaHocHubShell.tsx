...
import { useKhoaHocFilters } from "@/app/find-courses/_components/khoa-hoc-filters";
import { KhoaHocListingCard } from "@/app/find-courses/_components/KhoaHocListingCard";
import { TimKhoaHocKhoaFilterBar } from "@/app/find-courses/_components/TimKhoaHocKhoaFilterBar";
import type { TimKhoaHocLoai } from "@/app/find-courses/_components/tim-khoa-hoc-params";
import { TKH_KHOA_PAGE_SIZE } from "@/app/find-courses/_components/tim-khoa-hoc-page-size";
  ...
  function loadMore() {
    ...
    startTransition(async () => {
      try {
        ...
        if (q) qs.set("q", q);
        const res = await fetch(`/api/find-courses/courses?${qs}`, {
          cache: "no-store",
        });
        if (!res.ok) {
